...
import {
  AttendanceAPI,
  type AttendanceCircuitKeys,
  type AttendanceProviders,
  type DeployedAttendanceAPI,
} from '../../../api/src/index';
...
import { AttendancePrivateState } from '@midnight-ntwrk/attendance-contract';
...
export interface DeployedBoardDeployment {
  ...
  /**
   * The {@link DeployedAttendanceAPI} instance when connected to an on network bulletin board contract.
   */
  readonly api: DeployedAttendanceAPI;
}
...
export class BrowserDeployedBoardManager implements DeployedBoardAPIProvider {
  ...
  #initializedProviders: Promise<AttendanceProviders> | undefined;
  ...
  private getProviders(): Promise<AttendanceProviders> {
  ...
  private async deployDeployment(deployment: BehaviorSubject<BoardDeployment>): Promise<void> {
    try {
      ...
      const api = await AttendanceAPI.deploy(providers, this.logger);
  ...
  private async joinDeployment(
    ...
    try {
      const providers = await this.getProviders();
      const api = await AttendanceAPI.join(providers, contractAddress, this.logger);
...
/** @internal */
const initializeProviders = async (logger: Logger): Promise<AttendanceProviders> => {
  const networkId = import.meta.env.VITE_NETWORK_ID as NetworkId;
  const connectedAPI = await connectToWallet(logger, networkId);
  const zkConfigPath = window.location.origin; // '../../../contract/src/managed/attendance';
  const keyMaterialProvider = new FetchZkConfigProvider<AttendanceCircuitKeys>(zkConfigPath, fetch.bind(window));
  const config = await connectedAPI.getConfiguration();
  const inMemoryAttendancePrivateStateProvider = inMemoryPrivateStateProvider<string, AttendancePrivateState>();
  const shieldedAddresses = await connectedAPI.getShieldedAddresses();
  return {
    privateStateProvider: inMemoryAttendancePrivateStateProvider,
