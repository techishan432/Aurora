...
import { type AttendanceDerivedState, type DeployedAttendanceAPI } from '../../../api/src/index';
 ...
 * `DeployedAttendanceAPI` instance, upon which it will then subscribe to its `state$` observable in order
...
export const Board: React.FC<Readonly<BoardProps>> = ({ boardDeployment$ }) => {
  ...
  const [deployedBoardAPI, setDeployedBoardAPI] = useState<DeployedAttendanceAPI>();
  const [errorMessage, setErrorMessage] = useState<string>();
  const [boardState, setBoardState] = useState<AttendanceDerivedState>();
  ...
  // state, and we just need to forward it to the `post` method of the `DeployedAttendanceAPI` instance
  ...
  // of the `DeployedAttendanceAPI` instance.
  ...
  // Subscribes to the `state$` observable on a `DeployedAttendanceAPI` if we receive one, allowing the
